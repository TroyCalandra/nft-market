...
contract BuilderShop {
    ...
    function createNewBuilderInstance(
        ...
        uint256 num_nifties,
        uint256[] memory nifty_quantities,
        ...
        NiftyBuilderInstance new_contract =
            new NiftyBuilderInstance(
                _name,
                _symbol,
                contractId,
                num_nifties,
...
contract NiftyBuilderInstance is ERC721Full {
    ...
    mapping(uint256 => Counters.Counter) public _numNiftyMinted;
    mapping(uint256 => uint256) public _numNiftyPermitted;
    ...
    constructor(
        ...
        for (uint256 i = 0; i < (num_nifties); i++) {
    ...
    function isNiftySoldOut(uint256 niftyType) public view returns (bool) {
        ...
        if (
            _numNiftyMinted[niftyType].current() > _numNiftyPermitted[niftyType]
        ) {
            return (true);
        } else {
            return (false);
        ...
        onlyValidSender
    {
        ...
        uint256 tokenId =
            bm.encodeTokenId(contractId, niftyType, specificTokenId);
...
contract BuilderMaster {
    ...
    function getSpecificNiftyNum(uint256 tokenId) public view returns (uint256);
